...
with control_rail:
    with st.container(border=True):
        ...
        # key= (not value=) so the widget writes straight into session_state on the
        # same run: with value=, a new pick only lands on the NEXT rerun, which
        # reads as "the first click does nothing".
        st.select_slider("k", options=ks, key="slider_k")
        st.select_slider("time bucket (min)", options=time_buckets, key="slider_tb")
        st.selectbox("area mode", options=area_modes, key="slider_am")
        st.select_slider("epsilon", options=epsilons, key="slider_eps")
