...
def render_report(results: list[dict[str, Any]], ctx: Context) -> str:
    ...
    lines += [
        ...
        "## Verdict rules",
        "",
        "Turning any measurement above into pass / residual / fail is a judgement, so",
        "the judgement is written down rather than asserted per slide. These rules are",
        "implemented in `app/lib/verdicts.py`, which scores the app's release scoreboard",
        "directly from `outputs/risk_eval.json`.",
        "",
        "- **FAIL** - an individual is identifiable with probability greater than 1/k,",
        "  or an attack beats its baseline by more than the differential-privacy bound",
        "  allows.",
        "- **RESIDUAL** - the criterion holds, but with a named weakness.",
        "- **PASS** - neither of the above.",
        "",
        "The two releases are scored separately (`record` is row-level and k-anonymous",
        "with no DP; `aggregate` is counts only and is the release we recommend), each",
        "under the contextual and the simplified attacker model.",
        "",
        "**No Linkage on the aggregate release is a PASS by construction.** The release",
        "contains no records, so there is nothing to link, and the subscriber key was",
        "destroyed rather than hashed. A2's trajectory figure is therefore not a verdict",
        "on that release: it is the counterfactual that explains why linkage was removed",
        "in the first place, and it is reported under that heading. Linkage-shaped risk",
        "against the aggregate is measured under No Inference, by A5 and A6.",
        "",
        "## Wording rule",
        "",
